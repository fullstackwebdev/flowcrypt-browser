...
Catch.try(async () => {
  const uncheckedUrlParams = Env.urlParams([
    "acctEmail",
    "parentTabId",
    "bugReport"
  ]);
  const acctEmail = Env.urlParamRequire.optionalString(
    uncheckedUrlParams,
    "acctEmail"
  );
  const parentTabId = Env.urlParamRequire.string(
    uncheckedUrlParams,
    "parentTabId"
  );
  const bugReport = Env.urlParamRequire.optionalString(
    uncheckedUrlParams,
    "bugReport"
  );
  ...
  if (bugReport) {
    ...
    $(".line.info").text(
      "Please describe in detail what were you doing. Does this happen repeatedly?"
    );
    $("#input_text").val(`\n\n\n--------- BUG REPORT ----------\n${bugReport}`);
  }

  $(".action_send_feedback").click(
      ...
      try {
        ...
        if (r.sent) {
          ...
          Ui.Swal.fire({
            position: "top-end",
            type: "success",
            title: `Message sent! You will find your response in ${myEmail}, check your email later. Thanks!`,
          ...
          alert(
            "There was an error sending message. Our direct email is human@flowcrypt.com"
          );
        }
      } catch (e) {
